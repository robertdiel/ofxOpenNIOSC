...
ofxSpeechSynthesizer::ofxSpeechSynthesizer()
{
    ...
    if (errorStatus == noErr) {
		//errorStatus = SetSpeechInfo(speechChannel, soSpeechDoneCallBack, OurSpeechDoneCallBackProc);
        errorStatus = SetSpeechInfo(speechChannel, soSpeechDoneCallBack, (const void *)OurSpeechDoneCallBackProc);
        /*
        bool enable = true;
        OSErr       errorStatus;
        OSType      theMode;
